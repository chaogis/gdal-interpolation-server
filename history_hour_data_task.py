...
def work(current_task_name, use_history_temp = False):
    ...
    grid.start('hour', current_task_name, use_history_temp)
    exportpng.start('hour', current_task_name, use_history_temp)
...
def run_task(func, day=0, hour=0, min=0, second=0, start = None, end = None, use_history_temp = False):
    try:
        ...
        log = []
        log.append('>>>>>>>>>>' + format_time() +' 生成小时历史数据任务[ ' + task_name  + u' ]插值异常:\n')
...
def do_hour_task(start_time, end_time, type = None, use_history_temp = False):
    if start_time is not None and end_time is not None:
        if start_time <= end_time:
            ...
            run_task(work, hour = 1, start = start_time, end = end_time, use_history_temp = use_history_temp)
        else:
